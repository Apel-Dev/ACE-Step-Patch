            ...
            if torch.cuda.is_available():
                ...
                if major < 8:  # pre-Ampere (T4, P100, V100, etc.)
                    ...
                    if hasattr(dit_handler, 'model') and dit_handler.model is not None:
                        dit_handler.model = dit_handler.model.float()
                        print(f"   ✅ DiT model cast to float32")
                    if hasattr(dit_handler, 'silence_latent') and dit_handler.silence_latent is not None:
                        dit_handler.silence_latent = dit_handler.silence_latent.float()
                        print(f"   ✅ silence_latent cast to float32")
                    # Also cast text_encoder and vae dtype references
                    print(f"   ✅ dit.dtype = {dit_handler.dtype}")
